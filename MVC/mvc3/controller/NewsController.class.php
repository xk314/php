	...
	final class NewsController extends BaseController{
		...
		public function delete(){
			...
			if($this->NewsModel->delete($id))
				$this->redirect("NID = "."$id 删除成功！");
			else $this->redirect("NID = "."$id 删除失败！");		
		}
		...
		public function update(){
			...
			if($this->NewsModel->update($arr, $id))
				$this->redirect("NID = "."$id 修改成功！"); //变量名和汉字连起来的话 会将其整体视为一个变量名
			else $this->redirect("NID = "."$id 修改失败！");
		}
		...
		public function insert(){
			...
			if($this->NewsModel->insert($arr))
				$this->redirect("添加新闻成功！"); 
			else
				$this->redirect("添加新闻失败！");
		}
