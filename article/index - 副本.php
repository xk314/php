...
<body>
	...
	<!-- 分页显示可重用 -->
		<ul class="pager">
			...
			<li><a href="./index.php?page=<?php echo $now_page_index==1? 1:$now_page_index-1?>">上一页</a></li>
			<?php for($i=1; $i<=$total_page_num; $i++):?>
				<?php if($i>$total_page_num ):break;endif?>
				<?php if(abs($i-$now_page_index)<=3 ||($i<=7 && $now_page_index<4)):?>
					<li ><a id="<?=$i?>" href="./index.php?page=<?=$i?>"><?=$i?></a></li>
				<?php endif?>
			<?php endfor?>
			<li><a href="./index.php?page=<?php echo $now_page_index==$total_page_num? $total_page_num:$now_page_index+1?>">下一页</a></li>
			<li><a href="./index.php?page=<?=$total_page_num?>">尾页</a></li>
		</ul>
	</div>
</body>
</html>
