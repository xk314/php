...
final class Page {
	...
	public function show_page(){
		$str = "<ul  class='show' class='pager'>";
		$str .= "<li class='show'><a href='$this->url"."&page=1'>首页</a></li>";
		$page_before_index = $this->now_page_index==1 ? 1:$this->now_page_index-1;
		
		$str .= "<li class='show'><a href='$this->url"."&page=$page_before_index'>上一页</a></li>";
		echo $str;
		for($i=1; $i<=$this->page_total_num; $i++){
				if($i>$this->page_total_num ) break;
				if(abs($i-$this->now_page_index)<=3 ||($i<=7 && $this->now_page_index<4))
					echo "<li class='show'><a id=$i href=$this->url"."&page=$i>$i</a></li>";
		}
		$page_next_index = $this->now_page_index==$this->page_total_num? $this->page_total_num:$this->now_page_index+1;
		echo "<li class='show'><a href='$this->url"."&page=$page_next_index'>下一页</a></li>";
		echo "<li class='show'><a href='$this->url"."&page=$this->page_total_num'>尾页</a></li></ul>";
